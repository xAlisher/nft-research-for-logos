...
| 2 | **Encrypted-payload storage** — the token NFT holding only carries `owned:bool`; the `{link,note}` ciphertext needs a home + key-wrapping to the owner's viewing key | **RESOLVED + PROVEN** (see [`../experiments/sealed-collection/ENCRYPT-THEN-SHIELD.md`](../experiments/sealed-collection/ENCRYPT-THEN-SHIELD.md)) | transport = **on-chain `metadata.uri`** (borsh+hex blob, 2818 B « 100 KiB cap), round-trips `TokenMetadata↔Data`, only recipient viewing key decrypts; encrypt-then-shield also passes | Done for link+note payloads. Storage/Codex only if a future payload exceeds 100 KiB (large media) — fallback, needs a VPS host |
