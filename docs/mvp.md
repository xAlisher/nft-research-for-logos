...
- **B0. ✅ PROVEN (2026-09-03).** The NFT-through-privacy path is verified: a real `NftPrintedCopy` private-transfers through the privacy circuit and the proof verifies against state, recording only private commitments (sender `owned:false`, recipient `owned:true`) — no public owner, no edge. Evidence + reproduce: [`../experiments/nft-privacy-proof/PROOF.md`](../experiments/nft-privacy-proof/PROOF.md). The riskiest assumption is resolved; B1–B4 are wallet plumbing over a proven path, not new crypto.
